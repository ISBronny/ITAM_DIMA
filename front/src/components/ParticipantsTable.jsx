    ...
    useEffect(() => {
        if (state.isLoading)
            fetch(`${process.env.REACT_APP_BACKEND_URL}/participants/`, {
                ...
                .then(x => x.json())
                .then(json => {
                    setState({...state, isLoading: false, participantsRows: json})
                })
    }, [state]);
            ...
            <div className="flex items-top justify-center h-screen pt-10">
                <div className="max-w-[100%] rounded overflow-hidden w-full">
                    <table className="w-full shadow">
                        ...
                        <div>
                            {state.isLoading ? "" : state.participantsRows.map(h => <ParticipantsTableRow
                                name={h.name}
                                date={h.skillsStack}
...
const ParticipantsTableRow = ({name, skillsStack, entriesNumber, winsNumber}) => {
    return (
        <tr className="border-b border-gray-200 dark:border-gray-900">
            ...
            <td className="py-4 sm:pl-6 pl-4">
                <div className="flex items-center">
                    ...
                    <p className="text-sm leading-tight text-gray-500 dark:text-gray-400 pl-3">{skillsStack}</p>
                </div>
            </td>
            <td className="py-4 sm:pl-6 pl-4">
